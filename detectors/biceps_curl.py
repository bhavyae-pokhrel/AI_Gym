...
class BicepsCurlDetector(BaseExercise):
    ...
    def _safe_angle(self, dx, dy):
        return math.degrees(math.atan2(abs(dx), abs(dy))) if dy != 0 else 0.0 #!return angle from the VERTICAL (Y-axis) instead of horizontal.


#LEFT_SHOULDER   •────────• RIGHT_SHOULDER
#                   MID_S(x,y)             ← midpoint of shoulders
#                     |
#                     |  from these 2 point we find safe_angle
#                     |
#                   MID_H(x,y)             ← midpoint of hips
# LEFT_HIP       •────────• RIGHT_HIP
