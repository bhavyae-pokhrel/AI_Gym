...
#^ Knee angle < 100°  This means the user has gone low enough in the squat. DOWN_THRESHOLD = 100 When the knee angle drops below 90°, you can mark: 
# stage = "down"
#^ Knee angle > 160° This means the user has stood back up completely.     UP_THRESHOLD  = 160 When the angle rises above 160° after being in the "down" stage:
#  if stage == "down" and angle > UP_THRESHOLD:  (stage == "down" written to avoid multiple times increase reps)
#    reps += 1
#    stage = "up"

#^ reps increase when the user returns to the original/start position after completing the movement.
#^ In Squard rep increases when user come down -> up while in shoulder-push when user come up -> down
