...
# #! _ used before name means it is used only in this file it does not export to other file(private name/function) better for readibility
# _DB_PATH = str(Path(__file__).parent.parent.parent / "data.db") # parent.parent.parent => persistence-> services-> 08_AI_GYM

# #! check_same_thread=False → allows multiple threads to share same db connection → required in Streamlit because it is multi-threaded
# @st.cache_resource
# def _get_connection():
#     conn = sqlite3.connect(_DB_PATH,check_same_thread=False) 
#     conn.row_factory = sqlite3.Row #! instead access through index row[0],row[1] we want to access through column row[name], row[age]
#     return conn
